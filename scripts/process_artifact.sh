...
p=$(free_port); PORT=$p "$tmp/service" never-healthy >/dev/null 2>&1 & pid=$!; pids=$pid; wait_port "$p"; [ "$(curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:$p/)" = 503 ]; kill -KILL $pid; wait $pid 2>/dev/null || true; pids=""
p=$(free_port); PORT=$p "$tmp/service" never-bind >/dev/null 2>&1 & pid=$!; pids=$pid; sleep .05; ! (echo >/dev/tcp/127.0.0.1/$p) 2>/dev/null; kill -KILL $pid; wait $pid 2>/dev/null || true; pids=""
